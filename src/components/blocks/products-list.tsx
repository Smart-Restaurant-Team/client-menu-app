          ...
          {products.map((product, index) => (
            <Card key={index} className={cn('border-none shadow-none', product.salePrice && 'relative')}>
              ...
              <CardContent className='flex flex-1 flex-col justify-between gap-6'>
                {/* Product Image */}
                <Link href={`/dishes/${product._id}/details`}>
                  <img src={'https://cdn.shadcnstudio.com/ss-assets/blocks/ecommerce/product-list/image-6.png'} alt={product.imgAlt} className='mx-auto size-50' />
                </Link>
                ...
                <div className='space-y-4'>
                  ...
                  <div className='flex items-center justify-between'>
                    {!product.salePrice && <span className='text-2xl font-semibold'>${product.price && product.price.toFixed(2)}</span>}
                    {product.salePrice && (
